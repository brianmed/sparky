...
my %ENTITIES;
my $datafile = "entities.txt";
my $filename = $PerlApp::VERSION && !$^C ? PerlApp::extract_bound_file($datafile) : catfile(dirname(__FILE__), 'entities.txt');
for my $line (split "\x0a", slurp($filename)) {
