...
sub add_receive_socket
{
	...
	$self->{MULTICAST_LISTEN_SOCKET}->mcast_if($CONFIG{'LOCAL_IPADDR'});
	$self->{MULTICAST_LISTEN_SOCKET}->mcast_loopback(0);
	$self->{MULTICAST_LISTEN_SOCKET}->mcast_add($self->{MULTICAST_GROUP}) || PDLNA::Log::fatal('Cannot bind to SSDP listening socket: '.$!);
}
...
sub receive_messages
{
	my $self = shift;
		...
		if ($client_allowed)
		{
			PDLNA::Log::log('Received SSDP message from allowed client IP '.$peer_ip_addr.'.', 3, 'discovery');
		}
		else
		{
		...
		if ($message{'TYPE'} eq 'NOTIFY')
		{
			...
			{
				PDLNA::Log::log('Ignore SSDP message from allowed client IP '.$peer_ip_addr.', because the message came from this running '.$CONFIG{'PROGRAM_NAME'}.' installation.', 3, 'discovery');
				return;
			}
