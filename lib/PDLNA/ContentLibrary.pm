...
sub process_directory
{
	...
	foreach my $element (sort @elements)
	{
		...
		elsif (-f "$element") # && !grep(/^\Q$element_basename\E$/, @{$$params{'exclude_items'}}))
		{
			...
			PDLNA::Log::log('Processing '.$element.' with MimeType '.$mime_type.'.', 3, 'library');
            ...
            elsif (1 || PDLNA::Media::is_supported_mimetype($mime_type))
            {
				my $media_type = PDLNA::Media::return_type_by_mimetype($mime_type);
				if ($media_type && ($media_type eq $$params{'type'} || $$params{'type'} eq "all"))
				{
					PDLNA::Log::log('Adding '.$media_type.' element '.$element.'.', 3, 'library');
...
sub cleanup_contentlibrary
{
	...
	foreach my $file (@files)
	{
		...
		{
			### PDLNA::Log::log("Removing $$file{FULLNAME}", 1, "library");
	...
	foreach my $directory (@directories)
	{
		...
		{
			### PDLNA::Log::log("Removing $$directory{PATH}", 1, "library");
...
### 		{
### 			PDLNA::Log::log("Removing $$external{NAME}", 1, "library");
